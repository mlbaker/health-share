...
ARG password="isc"
ARG cache=HS-2017.1.0.792.0-hscore15.021_hsaa15.02_hspi15.021_hsviewer15.02_linkage15.021-b3077

ENV TMP_INSTALL_DIR=/tmp/distrib

# vars for Caché silent install
ENV ISC_PACKAGE_INSTANCENAME="HS2017" \
    ISC_PACKAGE_INSTALLDIR="/opt/hs2017" \
...
# Caché container main process PID 1 (https://github.com/zrml/ccontainermain)
 ...
 && ccontrol stop $ISC_PACKAGE_INSTANCENAME quietly \
 && sed -i 's/^MaxServerConn=1/MaxServerConn=10/' /opt/hs2017/cache.cpf \
...
ENTRYPOINT ["/ccontainermain", "-cconsole", "-i", "hs2017"]